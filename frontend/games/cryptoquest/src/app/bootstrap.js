...
const persistence = new PersistenceAdapter({ key: 'cryptoquest:architecture:v2', schemaVersion: 2 });
const api = new ApiClient({ baseUrl: '' });
const initialState = persistence.load({
  player: null,
  ui: { screen: 'boot', previousScreen: null },
  runtime: { legacyDetected: false },
  meta: { bootedAt: Date.now(), architectureVersion: 2 },
});
...
const services = Object.freeze({ navigation, energy, inventory });
...
window.addEventListener('beforeunload', () => scheduler.dispose(), { once: true });
