export class EnergyService {
  constructor({ store, bus, now = () => Date.now() }) {
    this.store = store;
    this.bus = bus;
    this.now = now;
  }
  ...
  canSpend(cost) {
    const energy = this.store.select(s => s.player?.energy);
    return Boolean(energy && Number.isFinite(cost) && cost >= 0 && energy.current >= cost);
  }

  spend(cost, reason = 'action') {
    if (!this.canSpend(cost)) return false;
    this.store.update(state => {
      state.player.energy.current = Math.max(0, state.player.energy.current - cost);
      state.player.energy.updatedAt = this.now();
    }, { source: 'energy-service', reason, cost });
  ...
  restore(amount, reason = 'recovery') {
    ...
    this.store.update(state => {
      ...
      energy.updatedAt = this.now();
    }, { source: 'energy-service', reason, amount });
