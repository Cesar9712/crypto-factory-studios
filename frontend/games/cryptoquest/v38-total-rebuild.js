...
function classAsset(){const t=(text(document.querySelector('.home-identity span'))||text(document.querySelector('.player-hud .identity span'))||'GUERRERO').toUpperCase();return AS+'v34-hero-warrior.svg'}
...
function getLevel(screen){const candidates=['.home-avatar-level','.home-avatar .level','.home-avatar','[data-level]','.player-hud .avatar','.player-hud .identity'];for(const sel of candidates){const el=screen.querySelector(sel)||document.querySelector(sel);const n=num(text(el),'');if(n)return n}return '1'}
function buildHome(screen){
 if(screen.dataset.cq38==='1')return;screen.dataset.cq38='1';
 const name=text(screen.querySelector('.home-identity strong'))||text(document.querySelector('.player-hud .identity strong'))||'HÉROE';
 const role=text(screen.querySelector('.home-identity span'))||text(document.querySelector('.player-hud .identity span'))||'AVENTURERO';
 const lvl=getLevel(screen);
 ...
 const resources=make('div','cq38-resources',`<div class="cq38-resource">${SVG('energy')}<span><small>ENERGÍA</small><b>${r.energy}</b></span></div><div class="cq38-resource">${SVG('gold')}<span><small>ORO</small><b>${r.gold}</b></span></div><div class="cq38-resource">${SVG('gem')}<span><small>CRISTALES</small><b>${r.premium}</b></span></div>`);hub.append(resources);
 ...
 const left=make('div','cq38-rail left'),right=make('div','cq38-rail right');left.append(button('MISIONES','more','quest',findBtn(screen,/mision/i)),button('LOGROS','more','exp',findBtn(screen,/logro/i)),button('TIENDA','more','shop',findBtn(screen,/tienda/i)));right.append(button('COFRE','more','loot',findBtn(screen,/cofre|recompensa/i)),button('EVENTOS','more','arcane',event),button('MÁS','more','more',findBtn(screen,/^MÁS$/i)));hub.append(left,right);
 const missionBox=make('div','cq38-mission',`<small>HISTORIA PRINCIPAL</small><h3>${mission}</h3><p>${objective}</p><div class="cq38-mission-reward"><span>RECOMPENSA</span><b>AVANZAR</b></div>`);const continueSrc=findBtn(screen,/CONTINUAR AVENTURA|CONTINUAR/i);const c=continueSrc?continueSrc.cloneNode(true):button('CONTINUAR','adventure','sword');c.className='cq38-continue';c.textContent='CONTINUAR';missionBox.append(c);hub.append(missionBox);
 ...
 const grid=make('div','cq38-feature-grid');grid.append(button('MAPA','adventure','map'),button('MAZMORRA','more','dungeon'),button('HABILIDADES','hero','talents'),button('PERSONAJE','hero','hero'));hub.append(grid);screen.append(hub);
}
...
function syncState(root){const home=[...root.querySelectorAll('.home-screen')].find(el=>el.offsetParent!==null)||root.querySelector('.home-screen');const homeActive=!!home;document.documentElement.classList.toggle('cq38-home-active',homeActive);document.body?.classList.toggle('cq38-home-active',homeActive);if(home)buildHome(home)}
function refresh(){document.documentElement.classList.add('cq38-active');document.body?.classList.add('cq38-active');const root=document.getElementById('game');if(!root)return;syncState(root);enhanceHero(root);ensureInventory(root);ensureCombat(root);window.CQTotalRebuild={version:'38.0.1',checkpoint:'checkpoint/cryptoquest-v37-before-total-rebuild-20260829'}}
let p=false;const queue=()=>{if(p)return;p=true;requestAnimationFrame(()=>{p=false;refresh()})};const start=()=>{refresh();new MutationObserver(queue).observe(document.getElementById('game')||document.documentElement,{subtree:true,childList:true,characterData:true,attributes:true,attributeFilter:['class','style']})};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
})();