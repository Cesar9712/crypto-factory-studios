...
function patchCopy(){
 ...
 const reset=document.querySelector('#reset');if(reset){reset.hidden=true;reset.setAttribute('aria-hidden','true');reset.tabIndex=-1}
...
function applyBusyState(){
 document.body.classList.toggle('nexus-busy',busy);
 document.querySelectorAll('[data-action]').forEach(el=>{
   if(busy&&!el.disabled){el.disabled=true;el.dataset.nexusAutoDisabled='1'}
   else if(!busy&&el.dataset.nexusAutoDisabled==='1'){el.disabled=false;delete el.dataset.nexusAutoDisabled}
 });
}
function soon(){setTimeout(patchCopy,60)}
window.addEventListener('nexus:state',soon);
window.addEventListener('pageshow',soon);
window.addEventListener('nexus:busy',e=>{busy=Boolean(e.detail?.busy);applyBusyState()});
document.addEventListener('click',soon,true);
setTimeout(patchCopy,200);
