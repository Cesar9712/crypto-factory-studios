...
class PaymentMethodRegistry:
    def __init__(self, settings: Any):
        ...
        self._methods = {
            ...
            'usdt_bsc': PaymentMethod('usdt_bsc','BSC-USD','BNB Smart Chain','BEP-20',settings.bsc_usdt_address,BSC_USDT_CONTRACT,18,True,False,'USDT-compatible · BNB Smart Chain (BEP-20)','This route uses Binance-Peg BSC-USD. Production remains locked until final operator validation.'),
...
class PriceService:
    ...
    def quote_amount(self, usd_price: Decimal, method: PaymentMethod) -> tuple[Decimal, Decimal, str]:
        ...
        sol_usd=Decimal(str(self.settings.mock_sol_usd_rate))
        amount=(usd_price/sol_usd).quantize(Decimal('0.000000001'), rounding=ROUND_DOWN)
        return amount, sol_usd, 'mock_fixed_rate'

class MockBlockchainVerifier:
    def verify(self, method: PaymentMethod, txid: str, expected_amount: Decimal) -> dict[str,Any]:
        tx=txid.strip()
        base={'txid':tx,'network':method.network,'recipient':method.address,'asset':method.asset,'token_contract':method.token_contract,'confirmations_ok':True,'success':True}
