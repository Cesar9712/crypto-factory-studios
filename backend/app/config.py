...
class Settings:
    ...
    public_base_url: str = field(default_factory=lambda: os.getenv('CFS_PUBLIC_BASE_URL','https://crypto-factory-studios.cryptofactorystudios.workers.dev').strip())
    webhook_base_url: str = field(default_factory=lambda: os.getenv('CFS_WEBHOOK_BASE_URL','').strip())
    antivirus_required: bool = field(default_factory=lambda: os.getenv('CFS_EXTERNAL_ANTIVIRUS_REQUIRED','false').lower()=='true')
    tropipay_enabled: bool = field(default_factory=lambda: os.getenv('CFS_TROPIPAY_ENABLED','false').lower()=='true')
    tropipay_environment: str = field(default_factory=lambda: os.getenv('CFS_TROPIPAY_ENV','sandbox').lower().strip())
    tropipay_api_base_url: str = field(default_factory=lambda: os.getenv('CFS_TROPIPAY_API_BASE_URL','https://sandbox.tropipay.me/api/v3').strip())
    tropipay_client_id: str = field(default_factory=lambda: os.getenv('CFS_TROPIPAY_CLIENT_ID','').strip())
    tropipay_client_secret: str = field(default_factory=lambda: os.getenv('CFS_TROPIPAY_CLIENT_SECRET','').strip())
    tropipay_currency: str = field(default_factory=lambda: os.getenv('CFS_TROPIPAY_CURRENCY','USD').upper().strip())
    tropipay_timeout_seconds: float = field(default_factory=lambda: float(os.getenv('CFS_TROPIPAY_TIMEOUT_SECONDS','10')))
