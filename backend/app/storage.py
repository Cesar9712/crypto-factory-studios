...
class StorageService:
    ...
    def _verify_s3_read_write(self)->bool:
        key=f'_cfs_health/{secrets.token_hex(8)}.probe'; payload=b'cfs-storage-read-write-check'
        try:
            self.client.head_bucket(Bucket=self.settings.s3_bucket); self.client.put_object(Bucket=self.settings.s3_bucket,Key=key,Body=payload,ContentType='application/octet-stream')
            return self.client.get_object(Bucket=self.settings.s3_bucket,Key=key)['Body'].read()==payload
        except Exception: return False
        finally:
            try: self.client.delete_object(Bucket=self.settings.s3_bucket,Key=key)
    ...
    def store_archive(self, build_id:str, data:bytes)->str:
        ...
        key=f'quarantine/{build_id}.zip'; self.client.put_object(Bucket=self.settings.s3_bucket,Key=key,Body=data,ContentType='application/zip'); return f's3://{self.settings.s3_bucket}/{key}'

    def read_archive(self, ref:str)->bytes:
        if not ref.startswith('s3://'): return Path(ref).read_bytes()
        return self.client.get_object(Bucket=self.settings.s3_bucket,Key=ref.split('/',3)[3])['Body'].read()

    def publish_zip(self, game_id:str, build_id:str, data:bytes, scanner)->None:
        manifest=scanner.validate_zip(data); prefix=manifest.get('strip_prefix')
        if self.backend=='local': scanner.safe_extract(data,self.settings.published_dir/game_id/build_id); return
        root=f'published/{game_id}/{build_id}/'
        with zipfile.ZipFile(io.BytesIO(data)) as z:
            for info in z.infolist():
                if info.is_dir(): continue
                p=scanner._normalized_path(info.filename)
                if prefix:
                    if not p.parts or p.parts[0].casefold()!=prefix.casefold(): raise ValueError('ambiguous_build_root')
                    if len(p.parts)==1: continue
                    p=PurePosixPath(*p.parts[1:])
                body=z.read(info); ctype=mimetypes.guess_type(str(p))[0] or 'application/octet-stream'
    ...
    def get_published(self, game_id:str, build_id:str, asset_path:str):
        ...
        try: obj=self.client.get_object(Bucket=self.settings.s3_bucket,Key=key)
    ...
    def delete_archive(self, ref:str)->None:
        ...
        if not ref.startswith('s3://'):
            ...
            return
        self.client.delete_object(Bucket=self.settings.s3_bucket,Key=ref.split('/',3)[3])
    ...
    def _delete_s3_prefix(self,prefix:str)->None:
        ...
        while True:
            ...
            page=self.client.list_objects_v2(**kwargs); keys=[{'Key':o['Key']} for o in page.get('Contents',[])]
    ...
    def delete_game(self,game_id:str,archive_refs:list[str]|tuple[str,...]=())->None:
        for ref in archive_refs:self.delete_archive(ref)
        if self.backend=='local':shutil.rmtree(self.settings.published_dir/game_id,ignore_errors=True);return
        self._delete_s3_prefix(f'published/{game_id}/')

    def ping(self)->bool:
        if self.backend=='local':return True
        if not self._startup_verified:return False
        try:self.client.head_bucket(Bucket=self.settings.s3_bucket);return True
        except Exception:return False
